...
var Freezer$1 = /*#__PURE__*/Object.freeze({
...
var Store = /** @class */ (function () {
    function Store(initialState, options, persistenceDriver) {
        ...
        this.initialState = new Freezer$1({ state: initialState });
        this.frozenState = new Freezer$1({ state: currentState }, this.opts);
    ...
    Store.prototype.saveDump = function () {
        var timestamp = this.persistenceDriver.saveDump(this.persistenceDriver.pack(this.frozenState.get().state));
        this.eventManager.fire(StoreEventType.DumpUpdate, this.frozenState.get().state, this.frozenState.get().state);
        return timestamp;
    };
    ...
    Store.prototype.setState = function (newState) {
        var newFrozenState = new Freezer$1(newState);
    ...
    Store.prototype.on = function (eventType, callback) {
        ...
        this.eventManager.fire(StoreEventType.DumpUpdate, this.frozenState.get().state, this.frozenState.get().state, event);
        return event;
    };
    return Store;
}());
...
var followStore = function (store) { return function (WrappedComponent) {
    var Component$1 = /** @class */ (function (_super) {
        ...
        Component.prototype.componentWillMount = function () {
            var _this = this;
            this.storeEvent = store.on(StoreEventType.All, function (storeState, prevState, type) {
                _this.forceUpdate();
            });
        };
...
function useStore(store, options) {
    ...
    var _a = useState(options.mapper ? options.mapper(store.state) : store.state), state = _a[0], setState = _a[1];
    useEffect(function () {
        var storeEvent = store.on(options.eventType || StoreEventType.Update, function (storeState) {
            setState(options.mapper ? options.mapper(storeState) : storeState);
        });
        return function () {
            storeEvent.remove();
        };
...
export { Store, StoreComponent, StoreEvent, StoreEventType, StorePersistentDriver, StorePersistentLocalStorageDriver, followStore, useStore };
