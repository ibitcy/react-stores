...
function useStore(store, options) {
    ...
    var getStoreState = React.useCallback(function (storeState) { return (options.mapper ? options.mapper(storeState) : storeState); }, [options.mapper]);
    var _a = React.useState(getStoreState(store.state)), state = _a[0], setState = _a[1];
    React.useEffect(function () {
        var storeEvent = store.on(options.eventType || StoreEventType.Update, function (storeState) {
            setState(getStoreState(storeState));
        });
        return function () {
            storeEvent.remove();
        };
